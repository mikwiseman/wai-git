...
### Fixed
- Status messages no longer leak into commit messages (output redirected to stderr)

## [1.2.3] - 2026-01-19

### Added
- Multi-line commit messages for large/complex changes (subject + body with bullet points)

### Changed
- Increased max_tokens from 100 to 500 for OpenAI and Claude
- AI prompt now instructs to add body for complex changes
- Removed newline stripping from AI response processing

## [1.2.2] - 2026-01-19

### Fixed
- AI now sees ALL changed files in large commits (was truncated at 10KB)
- Commit messages now accurately describe changes across 10+ files

### Changed
- `get_diff()` now uses smart per-file extraction (3KB/file, 40KB total)
- Extracts only `+`/`-` lines for compact, informative diffs
